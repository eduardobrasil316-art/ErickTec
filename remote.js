...
(function(window){
  ...
  // Map our resource names to DB table names (add `cadastros`)
  const TABLE_MAP = { clients: 'clients', vehicles: 'vehicles', products: 'products', orders: 'orders', cadastros: 'cadastros' };
  ...
  const Remote = {
    ...
    async syncDownload(){
      ...
      try{
        ...
        if(Array.isArray(cadastros)) localStorage.setItem(STORAGE_KEYS.cadastros, JSON.stringify(cadastros));
